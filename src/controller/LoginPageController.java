		...
		try{ 
			...
			for(Node n: colorStripNodes){
				...
				for(Node p : colorStrip)
				{
					...
					if(colIndex==0)  ColorView.setColor(colorHolder, colorPairList.get(colorPairIndex).color1);
					else ColorView.setColor(colorHolder, colorPairList.get(colorPairIndex).color2);
					
					/*
					 if(colorIndex==1) ColorView.setColor(colorHolder, colorPairList.get(colorPairIndex).color1);
			...
			textAuthSubmitBtn.setOnAction(new EventHandler<ActionEvent>() { 
				...
				public void handle(ActionEvent e) { 
					...
					if(isAuthorized) { System.out.println("Authorized"); System.exit(0); }
